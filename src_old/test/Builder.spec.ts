...
describe('Builder', function() {
	...
	describe('BlueprintBlueprint Path tools/blueprints', function() {
		it('Should create two blueprints [multiple-dir,single-dir]', function() {
			...
			expect(builder.getBlueprintNames()).to.eql([
				'multiple-dir',
				'single-dir',
			]);
		});
		...
		it('Valid moduleNames should NOT thrown an error..  only characters, numbers and underscore allowed', function() {
			...
			validNames.map(moduleName => {
				expect(function() {
					...
					/only characters, numbers and underscore allowed/i,
				);
			});
			rimraf.sync(tempPath);
		});
	...
	describe('Build "single-dir" success', function() {
		it(
			'Should generate the missing directories and a list of files in the tools/temp directory. ' +
				'A new directory must be created with the name of the new module.',
			function() {
				rimraf.sync(tempPath);
				fsx.ensureDirSync(tempPath);
				let root = path.resolve(tempPath);
