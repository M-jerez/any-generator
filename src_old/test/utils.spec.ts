...
describe('util', function() {
	describe('replaceAll matches in  multiline string', function() {
		it('Should replace all matches of "__name__" in the View File', function() {
			let refFileV = fsx.readFileSync(
				'tools/reference/refItem/refItemView.ts',
				'utf8',
			);
			let srcFileV = fsx.readFileSync(
				'tools/blueprints/single-dir/__name__/__name__View.ts',
				'utf8',
			);
		...
		it('Should replace all matches of "__name__" in the Model File', function() {
			let refFileM = fsx.readFileSync(
				'tools/reference/refItem/refItemModel.ts',
				'utf8',
			);
			let srcFileM = fsx.readFileSync(
				'tools/blueprints/single-dir/__name__/__name__Model.ts',
				'utf8',
			);
		...
		it('Should replace all matches of "__name__" in the Controller File', function() {
			let refFileC = fsx.readFileSync(
				'tools/reference/refItem/refItemController.ts',
				'utf8',
			);
			let srcFileC = fsx.readFileSync(
				'tools/blueprints/single-dir/__name__/__name__Controller.ts',
				'utf8',
			);
	...
	describe('getSubDirectoryNames', function() {
		...
		it(
			'Should return an array with a list of the first level subdirectories (non recursive)' +
				'ie: module09, Test, user_admin, userAdmin',
			function() {
				expect(
					util.getSubDirectoryNames('./tools/blueprints'),
				).to.be.eql(['multiple-dir', 'single-dir']);
